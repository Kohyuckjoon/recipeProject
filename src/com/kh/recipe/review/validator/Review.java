...
public class Review {
	...
	public boolean test() {
		...
		// 리뷰 글자수 5-100자
		vaild = Pattern.matches("^[a-zA-Zㄱ-힣0-9$@$!%*#?&]{5,100}$", reviewContents);
		if(!vaild) {
			failedAttribute.put("reviewContents", reviewContents);
			res = false;
		}
		
		
		if (!res) {
			request.getSession().setAttribute("reviewFailed", failedAttribute);
			request.getSession().setAttribute("reviewForm", this);
			/* this는 public class JoinForm을 의미 */
		}else {
			request.getSession().removeAttribute("reviewFailed");
			request.getSession().removeAttribute("reviewForm");
