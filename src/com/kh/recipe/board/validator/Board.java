...
public class Board {
	...
	public Board(HttpServletRequest request) {
		...
		this.contents = request.getParameter("contents");
}
	...
	public boolean test() {
		...
		// 게시글 제목 글자수 5-20자
		vaild = Pattern.matches("^[a-zA-Zㄱ-힣0-9$@$!%*#?&]{5,20}$", title);
		if(!vaild) {
			failedAttribute.put("title", title);
			res = false;
		}

		// 게시글 내용 글자수 5-500자
		vaild = Pattern.matches("^[a-zA-Zㄱ-힣0-9$@$!%*#?&]{5,500}$", contents);
		if(!vaild) {
			failedAttribute.put("contents", contents);
			res = false;
		}
		
		
		if (!res) {
			request.getSession().setAttribute("boardFailed", failedAttribute);
